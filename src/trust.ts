...
export function sign(
  ...
  let wssecurity: 'wssecurity' | undefined = undefined
  ...
  const signatureAlgorithm =
    ...
    ('http://www.w3.org/2000/09/xmldsig#rsa-sha1' satisfies SignExtraOptions['signatureAlgorithm'])

  const canonicalizationAlgorithm =
    options?.canonicalizationAlgorithm ||
    ('http://www.w3.org/2001/10/xml-exc-c14n#' satisfies SignExtraOptions['canonicalizationAlgorithm'])
  ...
  const digestAlgorithm =
    ...
    ('http://www.w3.org/2000/09/xmldsig#sha1' satisfies SignExtraOptions['digestAlgorithm'])
  const transforms = [
    'http://www.w3.org/2000/09/xmldsig#enveloped-signature',
    // It seems that we need to always explciitly list something else in addition to enveloped-signature,
    // or the digest value will be invalid: https://github.com/node-saml/xml-crypto/issues/212
    canonicalizationAlgorithm
  ]
  ...
  signer.computeSignature(xml, {
    ...
    attrs: options?.attrs,
    existingPrefixes: options?.existingPrefixes,
...
export function verifySignature(xml: string, signature: XMLElement, key: pki.Certificate) {
  const signer = new SignedXml({
    publicCert: pki.certificateToPem(key)
  })
  signer.loadSignature(signature as string | Node)
